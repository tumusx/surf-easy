...
function createSurfIcon(color) {
  // Try to use canvas if available, otherwise return null to use emoji fallback
  try {
    ...
    // Canvas module not available, return null to use emoji fallback
    return null;
  }
}
...
function updateTrayIcon(color) {
  if (tray) {
    ...
    // Try to create canvas-based surf icon first
    const icon = createSurfIcon(color);
    
    if (icon) {
      // Successfully created canvas icon
      tray.setImage(icon);
      tray.setTitle(''); // Clear title when using image
    } else {
      // Fallback to larger wave emoji
